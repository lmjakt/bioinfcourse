...
for $i(0..$#a){ 
    ...
    ## $i of sequence $as.
    ## Hence first print out the full 
    ## $as sequence and a newline.
    print $as, "\n";
    ## and then print out the appropriate number of spaces. 
    ## So that $bs will start on the ith position.
    ## This is possible using the for $j(0..$i), but this will print out
    ## one too many spaces. There is a better way to loop through these
    ## values using the for(;;) type of loop.

    ## this loop has three parts: an initialisation of a loop variable ($j, set to 0 here)
    ## a condition which must be true for the loop to continue ($j must always be less than $i)
    ## and a statement to change the loop variable ($i++, i.e. increase the value of $i for
    ## each iteration of the loop. But it can be anything, $i--, $i+=10, $i = $i*2, and so on).
    ## i.e.:  for (initialisation of loop variable; condition that must be true; change of loop variable).
    for($j=0; $j < $i; $j++){
	print " ";
    }
    print $bs, "\n";
