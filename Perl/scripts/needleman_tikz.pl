...
for $i(0..$#seq2){
	...
	print "\t\\node [scale=$fsscale] at (", $j+3, ",", $sn2-$i, ") {$s[$mi[0]]};\n";
...
## to follow the path of the optimal alignment start at the end and read backwards
$i = scalar @seq2;
$j = scalar @seq1;
...
## this has been written to not use $n; as that should be a global
## variable if used as here..
sub find_path {
    my($i, $j, $n, $l) = @_;
